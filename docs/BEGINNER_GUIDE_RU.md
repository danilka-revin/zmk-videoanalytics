...
- **Модели** — состояние загрузки модели в worker, безопасный тест без обязательных Precision/Recall, production-активация и мульти-модельный контур с отдельными слотами для людей, касок, спецодежды, телефона, курения и зон;
