...
- **Модели** — состояние загрузки модели в worker, безопасный тест без обязательных Precision/Recall и с отдельным мягким порогом рамок, production-активация и мульти-модельный контур с отдельными слотами для людей, касок, спецодежды, телефона, курения и зон;
- **Ещё** — здоровье сервисов, индикаторы CPU/RAM/диска/GPU, ZIP-экспорт событий с русской таблицей и кадрами нарушений, а также отправка сводки смены через Telegram.

## 19. Альтернатива: бот в мессенджере MAX

Вместо Telegram можно запустить отдельного бота для MAX.

1. Запустите обычный стек. MAX можно выбрать даже без token: сервис начнёт работу в безопасном режиме ожидания и не затронет камеры, Telegram или аналитику.
2. Откройте **Админ → Боты → MAX** — встроенная карточка подскажет путь через `@MasterBot` и откроет официальную документацию MAX API.
3. Когда token выдаст MAX или владелец организации, вставьте его в карточку, добавьте свой числовой MAX user ID в «Администраторы ID» и включите провайдера:

```bash
./start.sh
```

Без официального token невозможно подключиться к инфраструктуре MAX от имени бота: ZMK Vision не генерирует и не обходит учётные данные платформы. Для headless-развёртывания можно вместо панели задать запасные `MAX_BOT_TOKEN`, `MAX_ADMIN_IDS`, `MAX_OPERATOR_IDS` и `MAX_VIEWER_IDS` в `.env`. Токен, сохранённый из Admin, имеет приоритет.

Бот MAX поддерживает состояние системы, камеры, события, логи, ZIP-отчёты с русской таблицей и доступными кадрами нарушений, модели, hot-swap, thresholds, обучение, отмену обучения и тестовые тревоги.
...
В Docker-версии вход в браузерную панель включён по умолчанию. Окно входа — обычная форма **«Имя пользователя + пароль»** (как в любом другом приложении): никакие готовые аккаунты не подставляются и не предлагаются.

Стартовая учётная запись одна — встроенный администратор:

- **Администратор** — логин **`admin`**, стартовый пароль **`admin`** (переменная `ZMK_INITIAL_PASSWORD`). Полный доступ: камеры, события, модели, обучение, диагностика, логи, администрирование, настройки и управление пользователями.

Нужны отдельные люди в панели? Администратор добавляет их в **Админ → Пользователи** (или в **Персонализация → Доступ → Пользователи панели**): логин, имя, роль и пароль. Созданный пользователь сразу входит через ту же стандартную форму. Ролей три:

- **Аналитик** — смотрит аналитику, камеры и события и **принимает или отклоняет детекции** (можно пакетом). Настройки площадки, камеры, модели и пользователи для него закрыты.
- **Гость** — только смотрит инфографику, камеры и события, без решений по событиям.
- **Администратор** — всё то же, что у встроенного `admin`.

Пользователя можно переименовать, перевести в другую роль, сбросить пароль, отключить (например, на время отпуска) или удалить. Смена роли применяется к его открытым сеансам сразу, а сброс пароля и отключение сразу завершают его сеансы.

Для директора и администратора на **«Обзоре»** и в **Админ → Аналитики** есть краткая сводка по аналитикам: кто сколько событий **принял**, сколько **не принял** и сколько **времени работал** (активные сессии — проверки с перерывами до 30 минут, длинные паузы не считаются). Период переключается на **24 ч / 7 дней / 30 дней**. Аналитик этой сводки не видит — ему она не нужна, он видит только свою очередь событий.

Кнопка **«Сменить пользователя»** в шапке панели и в блоке профиля слева открывает ту же стандартную форму логина прямо поверх работающей панели — фон при этом слегка размывается. Вход под другим аккаунтом (например, гость → администратор) завершает прежний сеанс этого браузера, а «Отмена» возвращает в панель без выхода. Пока вход не выполнен, панель видна только размытым фоном. Принудительная смена стартового пароля при первом входе отключена; при обновлении прежний не сменённый пароль (`1243` или `1234`) приводится к текущему `ZMK_INITIAL_PASSWORD`, а персональный пароль владельца не затрагивается.
