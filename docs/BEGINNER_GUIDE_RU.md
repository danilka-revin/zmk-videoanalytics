...
- **Главная** — фактический статус системы, worker и активная модель;
- **Камеры** — последние реальные кадры inference-worker, полный просмотр и перезапуск потока для admin;
- **События** — детали и кадры событий, подтверждение для operator/admin;
- **Модели** — состояние загрузки модели в worker и активация/тест для admin;
- **Ещё** — здоровье сервисов и CSV-отчёты.
