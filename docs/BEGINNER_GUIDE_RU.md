...
В Docker-версии вход в браузерную панель включён по умолчанию. Окно входа показывается по центру экрана и сначала предлагает выбрать пользователя:

- **Администратор** — стартовый пароль **`admin`** (переменная `ZMK_INITIAL_PASSWORD`). Полный доступ: камеры, события, модели, обучение, диагностика, логи, администрирование, настройки и управление пользователями.
- **Директор** — стартовый пароль **`director`** (переменная `ZMK_DIRECTOR_PASSWORD`). Только просмотр: инфографика «Обзор», камеры и события. Разделы моделей, диагностики, логов, администрирования и настроек скрыты, а API отвечает `403` на любые изменения. Персонализация внешнего вида доступна.

Нужны отдельные люди в панели? Администратор добавляет их в **Персонализация → Доступ → Пользователи панели**: логин, имя, роль и пароль. Ролей три:
