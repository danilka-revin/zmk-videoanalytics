...
В Docker-развёртывании парольный вход включён по умолчанию (`ZMK_PASSWORD_AUTH=true`). Окно входа открывается по центру экрана и сначала спрашивает, кто входит:
...
| **Директор** (`director`) | **`director`** (`ZMK_DIRECTOR_PASSWORD`) | Только просмотр: инфографика «Обзор», камеры и события. Технические разделы скрыты, а API отвечает `403` на любые изменения. Персонализация интерфейса доступна |

Администратор создаёт сколько угодно своих пользователей в **Персонализация → Доступ → Пользователи панели**. У каждого — логин, имя, роль и пароль:
...
Пользователя можно переименовать, перевести в другую роль, сбросить ему пароль, временно отключить или удалить; смена роли действует на его открытые сеансы сразу, а сброс пароля и отключение немедленно завершают его сеансы. Встроенного администратора удалить, отключить или разжаловать нельзя.

**Смена пользователя** (кнопка в шапке и в блоке профиля слева) не закрывает панель: список аккаунтов открывается поверх работающего интерфейса, который остаётся слегка размытым. Текущий аккаунт помечен — как в Steam: аватар, имя, роль и метка «Текущий» / «Вы вошли», а остальные показаны списком ниже. Вход под другим аккаунтом завершает прежний сеанс этого браузера. Без успешного входа фон панели остаётся размытым, а browser API-запросы не получают доступ к данным. Принудительная смена стартового пароля при первом входе больше не требуется (флаг `auth_password_must_change` по умолчанию `false`); в базах, где он установлен, экран смены пароля продолжает работать. При обновлении не сменённый владельцем стартовый пароль (`1243` или `1234`) автоматически приводится к текущему значению `ZMK_INITIAL_PASSWORD`; уже заданный владельцем пароль не меняется.
