...
![Version](https://img.shields.io/badge/version-1.2.0-25332d)
...
Версия `1.2.0` является рабочим демонстрационным и интеграционным контуром. Web-панель, REST API, администрирование, отчёты, Telegram-интерфейсы, hot-swap и жизненный цикл обучения работают локально. Реальное распознавание требует подключения камер, GPU, весов моделей и размеченного набора данных.
...
SQLite размещается в Docker volume. Пароли PostgreSQL и MinIO задаются только через `.env` и должны быть заменены перед production-запуском.
