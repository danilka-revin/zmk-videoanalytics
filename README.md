...
В Docker-развёртывании парольный вход включён по умолчанию (`ZMK_PASSWORD_AUTH=true`). Первый пароль — **`1243`**. После первого входа панель потребует заменить его; далее пароль и email для восстановления доступны в **Персонализация → Доступ**. Без успешного входа весь фон панели остаётся размытым, а browser API-запросы не получают доступ к данным.
