...
4. Запустите обычный стек:

```bash
./start.sh
```

После запуска включите Telegram в **Админ → Боты**.
...
3. Запустите обычный стек:

```bash
./start.sh
```

После запуска включите MAX в **Админ → Боты**.

После первого запуска оба bot-сервиса остаются в безопасном режиме ожидания. Включение/отключение провайдера, роли, получатели, минимальная критичность push и тест доставки выполняются в **Админ → Боты** без перезапуска контейнера. Токены намеренно остаются только в защищённом `.env` на сервере и не отображаются в веб-интерфейсе.
