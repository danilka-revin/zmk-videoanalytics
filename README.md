...
Каждый обычный `./start.sh` принудительно пересобирает образы (`up -d --build`),
в том числе web-стадию `npm ci`, которая ходит в реестр npm по сети. Если
`registry.npmjs.org` недоступен/медленный (частый случай для RU/CN), сборка web
может зависнуть без вывода. Два способа это обойти:

```bash
# 1) Быстрый перезапуск без полной пересборки (использует уже собранные образы;
#    компоуз достроит только отсутствующие). Код не менялся — просто перезапуск.
./start.sh --fast          # или: ZMK_FAST=1 ./start.sh

# 2) Если web-сборке всё же нужен свежий npm — укажите зеркальный реестр в .env
#    NPM_REGISTRY=https://registry.npmmirror.com
#    затем обычный ./start.sh
```

После реального обновления кода запускайтесь обычным `./start.sh`, чтобы образы
пересобрались с новым кодом.
