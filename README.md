...
При загрузке Precision/Recall можно **оставить пустыми**: модель зарегистрируется для безопасного **«Теста на камере»**, чтобы сначала увидеть реальные рамки своей нейросети. Параметры Quality gate в **Настройки** — это только минимальный порог для production, они не являются метриками вашей модели и не подставляются автоматически.
