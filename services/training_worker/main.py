...
class Job(BaseModel):
 id:int; camera_id:str; rtsp_url:str; target_name:str; base_artifact:str|None=None; image_count:int=Field(ge=20,le=5000); epochs:int=Field(ge=1,le=300); fps_limit:float=Field(default=2,gt=0,le=10)
...
def train(job:Job):
 ...
 for image in images.glob('*.jpg'):
  result=pseudo_model.predict(str(image),verbose=False,conf=.35)[0]; h,w=result.orig_shape; lines=[]
 ...
 for index,label in enumerate(labeled):
  split='val' if index%5==0 else 'train'; image=images/f'{label.stem}.jpg'; shutil.move(str(image),work/'images'/split/image.name); shutil.move(str(label),work/'labels'/split/label.name)
 ...
 trainer=YOLO(BASE_MODEL); result=trainer.train(data=str(data),epochs=job.epochs,imgsz=640,device=0,project=str(work/'runs'),name='train',exist_ok=True,verbose=False)
