...
docker compose --profile telegram --profile max --profile production down --remove-orphans
...
if ($Purge) {
  ...
  if ($answer -eq "DELETE") {
    docker compose --profile telegram --profile max --profile production down -v --remove-orphans
